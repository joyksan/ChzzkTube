...
### A3. [P2] 프리웜이 매 기동 시 `npm ci` + `tsc` 강제 재실행
...
### A5. [P3] F12 재오픈 시 닫혀 있던 구간 로그 미반영
...
### D1. [P1] `requirements.txt` — 방향성과 모순된 오래된 의존성
...
### D2. [P2] 루트 추적 잔재 파일 (git 저장소에 커밋됨)
...
### E2. [P3] `MainWindow.dl_state` 레거시 프로퍼티
- `ctrl.state` 별칭. `smoke_test.py` 호환용으로 유지 중 — 폐기 시 smoke 대사 갱신 필요.

### E3. [P3] `updater.py`의 "stdlib only" 표기 vs lazy `pot_provider`/`components` import
...
- 본 패치 기준: 전체 pytest **126 passed**, `python sync_mirrors.py --check` **변경 0건**, `py_compile` OK.
- 위 P1~P3 미반영 — 적용 여부/우선순위는 담당자 결정 사항.
- §7/수정 테이블의 `Main._maybe_prewarm_pot` — 코드에 없음 (실제는 `_on_update_check_done → POTManager.ensure_ready("prewarm")`).