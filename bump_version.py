...
try:
    ...
    if not match:
        print("[ERROR] config.py에서 APP_VERSION 패턴을 찾지 못했습니다!")
        sys.exit(1)
    ...
    if toml_count == 0:
        print("[ERROR] pyproject.toml에서 version 패턴을 찾지 못했습니다!")
        sys.exit(1)
    ...
    print(f"Version Bump 성공: v{old_ver} -> v{new_ver}")
