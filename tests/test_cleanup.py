...
def test_cleanup_provisioning_artifacts(monkeypatch, tmp_path):
    ...
    # ffmpeg .part 및 아카이브 파일 생성
    ffmpeg_dir = test_base / "ffmpeg"
    ffmpeg_dir.mkdir()
    (ffmpeg_dir / "ffmpeg.part").write_text("temp")
    (ffmpeg_dir / "ffmpeg.tar.xz").write_text("archive")
    (ffmpeg_dir / "ffmpeg.zip").write_text("archive")
    ...
    assert not (ffmpeg_dir / "ffmpeg.tar.xz").exists()
    assert not (ffmpeg_dir / "ffmpeg.zip").exists()
