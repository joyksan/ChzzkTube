...
> 마지막 갱신: 2026-09-16 - v3.6.1 — 아키텍처 다이어그램(mermaid) 추가 (문서 패치)
...
- **버전**: `v3.6.1` — 정의 위치 `config._APP_VERSION`; 메타 참고값은 `pyproject.toml` `version = "3.6.1"` (최신: 2026-09-16 아키텍처 다이어그램 추가)
...
- 앱이 표시하는 버전의 단일 진실 공급원은 `config._APP_VERSION`이다. 현재 값은 `v3.6.1`이다.
...
모든 다이얼로그는 `Cascadia Mono, 11px` 및 다크 팔레트(`#0d0d0d`)를 준수하며 모던 TUI 규칙에 따라 렌더링된다.
...
### 레이어별 구조 (4계층 + L0 Leaf) — v3.4.0 레이아웃 리팩터링(`chzzktube/` 단일 패키지)
