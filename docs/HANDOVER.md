...
> 마지막 갱신: 2026-09-26 - v3.12.1 — 의존성 통합 프로비저닝 파이프라인 일원화·Cold Boot Ready 게이트 복원·실시간 다운로드 게이지 정상화
---
## 목차
...
- **버전**: `v3.12.1` — 정의 위치 `config._APP_VERSION`; 메타 참고값은 `pyproject.toml` `version = "3.12.1"` (최신: 2026-09-26 의존성 통합 프로비저닝 파이프라인 일원화·Cold Boot Ready 게이트 복원·실시간 다운로드 게이지 정상화)
